...
export class EditorApp {
  ...
  start(): void {
    this.currentMap = structuredClone(this.options.initialMap ?? MapStorage.getLastMap() ?? createDefaultMap());
    this.renderShell();
  ...
  private bindMapInputs(): void {
    ...
    this.root.querySelector<HTMLButtonElement>("[data-capture-thumbnail]")
  ...
  private publishMap(): void {
    ...
    this.showToast("Mapa publicado localmente.");
  }
  ...
  private readEnvironmentInputs(): VisualSettings {
    ...
    return {
      ...
      fogNear: readNumberInput(this.root.querySelector<HTMLInputElement>("#fog-near-input"), fallback.fogNear),
      fogFar: readNumberInput(this.root.querySelector<HTMLInputElement>("#fog-far-input"), fallback.fogFar),
  ...
  private updateEnvironmentInputs(settings: VisualSettings | undefined): void {
    ...
    setInputValue(this.root.querySelector<HTMLInputElement>("#ground-color-input"), resolved.groundColor);
    setInputChecked(this.root.querySelector<HTMLInputElement>("#fog-enabled-input"), resolved.fogEnabled);
    setInputValue(this.root.querySelector<HTMLInputElement>("#fog-color-input"), resolved.fogColor);
    setInputValue(this.root.querySelector<HTMLInputElement>("#fog-near-input"), String(resolved.fogNear));
    setInputValue(this.root.querySelector<HTMLInputElement>("#fog-far-input"), String(resolved.fogFar));
    setInputValue(this.root.querySelector<HTMLInputElement>("#ambient-light-input"), String(resolved.ambientLightIntensity));
    setInputValue(this.root.querySelector<HTMLInputElement>("#sun-light-input"), String(resolved.sunLightIntensity));
  }
  ...
  private readAudioInputs(): AudioSettings {
    ...
    return {
      masterVolume: readPercentInput(this.root.querySelector<HTMLInputElement>("#master-volume-input"), fallback.masterVolume),
      sfxVolume: readPercentInput(this.root.querySelector<HTMLInputElement>("#sfx-volume-input"), fallback.sfxVolume),
      musicVolume: readPercentInput(this.root.querySelector<HTMLInputElement>("#music-volume-input"), fallback.musicVolume),
  ...
  private updateAudioInputs(settings: AudioSettings | undefined): void {
    ...
    setInputValue(this.root.querySelector<HTMLInputElement>("#master-volume-input"), String(Math.round(resolved.masterVolume * 100)));
    setInputValue(this.root.querySelector<HTMLInputElement>("#sfx-volume-input"), String(Math.round(resolved.sfxVolume * 100)));
    setInputValue(this.root.querySelector<HTMLInputElement>("#music-volume-input"), String(Math.round(resolved.musicVolume * 100)));
    setSelectValue(this.root.querySelector<HTMLSelectElement>("#ambient-music-input"), resolved.ambientMusic);
    setInputChecked(this.root.querySelector<HTMLInputElement>("#audio-muted-input"), resolved.muted);
    this.updateAudioOutputLabels(resolved);
  }

  private updateAudioOutputLabels(settings: AudioSettings | undefined): void {
    const resolved = resolveAudioSettings(settings);
    setOutputText(this.root.querySelector<HTMLOutputElement>("#master-volume-input-value"), `${Math.round(resolved.masterVolume * 100)}%`);
    setOutputText(this.root.querySelector<HTMLOutputElement>("#sfx-volume-input-value"), `${Math.round(resolved.sfxVolume * 100)}%`);
    setOutputText(this.root.querySelector<HTMLOutputElement>("#music-volume-input-value"), `${Math.round(resolved.musicVolume * 100)}%`);
  }
